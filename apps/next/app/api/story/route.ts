import { createStory, checkStorySlugExists } from '@repo/database'
import pool from '../../utils/open-pool'
import { requireAuth } from '../../../lib/auth-middleware'
import { NextResponse } from 'next/server'
import { generateUniqueSlug } from '../../../utils/slug'
import { CreateStorySchema } from '../../../lib/validations/story'
import { rateLimit, RateLimitConfigs } from '../../../lib/rate-limit'
import { checkQuotaWithClient } from '../../../lib/subscription'
import { z } from 'zod'

/**
 * Create a new story (authenticated users only)
 */
export async function POST(request: Request) {
  const rateLimitResult = await rateLimit(request, RateLimitConfigs.api)
  if (rateLimitResult) return rateLimitResult

  const authResult = await requireAuth(request)
  if (authResult.error) return authResult.error

  const { user } = authResult

  try {
    const body = await request.json()
    const validated = CreateStorySchema.parse(body)

    const client = await pool.connect()
    try {
      await client.query('BEGIN')

      // Advisory lock per user prevents concurrent quota bypass
      await client.query('SELECT pg_advisory_xact_lock(hashtext($1))', [
        `quota:${user.id}:stories`,
      ])

      const quotaCheck = await checkQuotaWithClient(client, user.id, 'stories')
      if (!quotaCheck.allowed) {
        await client.query('ROLLBACK')
        return NextResponse.json(
          {
            error: quotaCheck.reason,
            limit: quotaCheck.limit,
            count: quotaCheck.count,
            upgradeRequired: true,
          },
          { status: 403 },
        )
      }

      const slug = await generateUniqueSlug(
        validated.title,
        async (candidateSlug) => {
          const result = await checkStorySlugExists(client, {
            userid: user.id,
            slug: candidateSlug,
          })
          return result?.exists ?? false
        },
      )

      const storyRequest = await createStory(client, {
        userid: user.id,
        title: validated.title,
        content: validated.content,
        privacy: validated.privacy,
        slug,
      })

      await client.query('COMMIT')
      return NextResponse.json(storyRequest, { status: 201 })
    } catch (err) {
      await client.query('ROLLBACK').catch(() => {})
      throw err
    } finally {
      client.release()
    }
  } catch (error) {
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        { error: 'Validation failed', details: error.issues },
        { status: 400 },
      )
    }

    console.error(
      'Error creating story:',
      error instanceof Error ? error.message : 'Unknown error',
    )
    return NextResponse.json(
      { error: 'Failed to create story' },
      { status: 500 },
    )
  }
}
